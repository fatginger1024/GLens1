...
class cross_section():
    ...
    def get_critical_curves(self,xmin=-4,xmax=0):
        ...
        ind = np.argmin(div)
        # get 1st ind for theta > 0 on the left
        if np.all(div>0) or np.all(xval[:ind]<0):
            return self.get_critical_curves(xmin=xmin-2, xmax=0)
        ...
        try:
            ...
            print(self.stat.detA(10**xmin),self.stat.detA(xval[ind]),self.stat.detA(10**xmax))
    ...
    def run_get_loc(self, beg, end, mag_th=26.3,mu_th=1):
        if end-beg == 1:
        ...
        #if self.mag_unlensed>mag_th:
        if (self.get_lensed_mag(mag3_end) < mag_th) and (self.get_lensed_mag(mag3_mid) > mag_th):
            beg = mid
        elif (self.get_lensed_mag(mag3_end) < mag_th) and (self.get_lensed_mag(mag3_mid) < mag_th):
            end = mid
        #else:
        #    if (mag3_end > mu_th) and (mag3_mid < mu_th):
        #        beg = mid
        #    elif (mag3_end > mu_th) and (mag3_mid > mu_th):
        #        end = mid
        
        return self.run_get_loc(beg, end, mag_th=mag_th,mu_th=1)
